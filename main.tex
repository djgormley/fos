...
\end{equation*}
In words, the tone matched-filter GLRT statistic and the noise-normalized first-order cyclic-moment energy are the same quantity.  This equality holds at every evaluated $\alpha$; the condition $\alpha=k/N$ is needed only when the correlation is computed as an $N$-point DFT bin.  Read in reverse, $|\widehat R_{x,1}^{\alpha}(0)|=(\sigma/\sqrt{N})\sqrt{T_{\alpha}}$: $T_{\alpha}$ determines the magnitude of the complex cyclic-moment estimate, but not its phase.  Thus, under this model, the first-order cyclostationarity statistic is obtained by tone matched filtering followed by magnitude-squaring and noise normalization.
